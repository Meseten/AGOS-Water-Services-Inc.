...
const iconMap = {
    Home, UserCog, FileText, AlertTriangle, Users, MessageSquare, Settings, HelpCircle, Info, PhoneCall,
    LogOut, ShieldCheck, Megaphone, BarChart3, Edit, Gauge, Map, ClipboardEdit, Search, Banknote, FileSearch,
    TrendingUp, Gift, AlertIcon, EditIcon, Eye,
};
...
const Sidebar = ({
    ...
    const groupOrder = ['Main', 'Account', 'Operations', 'Administration', 'Support', 'Information', 'Help & Info'];
        ...
        <div className="h-full flex flex-col bg-gradient-to-b from-blue-700 to-blue-900 text-white shadow-2xl">
            ...
            <nav className="flex-grow p-3 space-y-1.5 overflow-y-auto scrollbar-thin scrollbar-thumb-blue-500 scrollbar-track-blue-800">
                {groupOrder.map(groupName => {
                    const items = groupedNavItems[groupName];
                    if (!items) return null;

                    return (
                        <div key={groupName} className="space-y-1.5">
                            {groupOrder.length > 1 && (
                                <h3 className="px-3.5 pt-3 pb-1 text-xs font-semibold text-blue-300 uppercase tracking-wider">
                                    {groupName}
                                </h3>
                            )}
                            {items.map(item => {
    ...
    return (
        <>
            <div
                className={`fixed inset-0 bg-black bg-opacity-60 z-40 lg:hidden transition-opacity duration-300 ${isMobileOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'} no-print`}
                onClick={() => setMobileOpen(false)}
                aria-hidden="true"
             ></div>

            <aside className={`fixed top-0 left-0 h-full w-64 z-50 transform transition-transform duration-300 ease-in-out lg:translate-x-0 ${isMobileOpen ? 'translate-x-0 shadow-xl' : '-translate-x-full'} no-print`}>
                {sidebarContent}
            </aside>
